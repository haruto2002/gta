...
def query_subtracks(seg1, seg2, track1, track2):
    ...
    while seg1 and seg2:  # Continue until seg1 or seg1 is empty
        s1_start, s1_end = seg1[
            0
        ]  # Get the start and end indices of the first segment in seg1
        s2_start, s2_end = seg2[
            0
        ]  # Get the start and end indices of the first segment in seg2
        ...
        if (
            s1_startFrame < s2_startFrame
        ):  # Compare the starting frames of the two subtracks
...
def get_spatial_constraints(tid2track, factor):
    ...
    for track in tid2track.values():
        for bbox in track.bboxes:
            ...
            x, y, w, h = bbox[
                0:4
            ]  # x, y is coordinate of top-left point of bounding box
...
def get_distance(track1_id, track2_id, track1, track2):
    ...
    if doesOverlap:
        ...
        track1_features_tensor = torch.tensor(
            np.stack(track1.features), dtype=torch.float32
        ).to(device)
        track2_features_tensor = torch.tensor(
            np.stack(track2.features), dtype=torch.float32
        ).to(device)
        count1 = len(track1_features_tensor)
        count2 = len(track2_features_tensor)

        track1_features_tensor = track1_features_tensor.squeeze(0)
        track2_features_tensor = track2_features_tensor.squeeze(0)
        # print(track1_features_tensor.shape)
        # print(track2_features_tensor.T.shape)
        cos_sim_Numerator = torch.matmul(
            track1_features_tensor, track2_features_tensor.T
        )
        track1_features_dist = torch.norm(
            track1_features_tensor, p=2, dim=1, keepdim=True
        )
        track2_features_dist = torch.norm(
            track2_features_tensor, p=2, dim=1, keepdim=True
        )
...
def merge_tracklets(
    ...
    while np.any(Dist[non_diagonal_mask] < merge_dist_thres):
        ...
        track1_idx, track2_idx = (
            masked_indices[0][min_index],
            masked_indices[1][min_index],
        )
        ...
        assert (
            min_value == Dist[track1_idx, track2_idx] == Dist[track2_idx, track1_idx]
        ), "Values should match!"

        track1 = tracklets[idx2tid[track1_idx]]
        track2 = tracklets[idx2tid[track2_idx]]

        inSpatialRange = check_spatial_constraints(
        ...
        if inSpatialRange:
            track1.features += (
                track2.features
            )  # Note: currently we merge track 2 to track 1 without creating a new track
            ...
            Dist[track1_idx, track2_idx], Dist[track2_idx, track1_idx] = (
                merge_dist_thres,
                merge_dist_thres,
            )
...
def detect_id_switch(embs, eps=None, min_samples=None, max_clusters=None):
    ...
    if -1 in labels and len(unique_labels) > 1:
        ...
        cluster_centers = np.array(
            [embs_scaled[labels == label].mean(axis=0) for label in unique_labels]
        )
    ...
    if max_clusters and n_clusters > max_clusters:
        ...
        while n_clusters > max_clusters:
            ...
            cluster_to_merge_1, cluster_to_merge_2 = (
                unique_labels[min_dist_idx[0]],
                unique_labels[min_dist_idx[1]],
            )
...
def split_tracklets(
    tmp_trklets, eps=None, max_k=None, min_samples=None, len_thres=None
):
    """
    ...
    for tid in tqdm(
        ...
        if (
            len(trklet.times) < len_thres
        ):  # NOTE: Set tracklet length threshold to filter out short ones
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Global tracklet association with splitting and connecting."
    )
    parser.add_argument(
        "--dataset",
        type=str,
    ...
    parser.add_argument(
        "--track_src",
        type=str,
        default="results/SNMOT-116/BoTSORT_Tracklets",
        # required=True,
        help="Source directory of tracklet pkl files.",
    )

    parser.add_argument(
        "--use_split", action="store_true", help="If using split component."
    )
    ...
    parser.add_argument(
        "--use_connect", action="store_true", help="If using connecting component."
    )
...
def main():
    ...
    for seq_idx, seq in enumerate(seqs_tracks):
        ...
        if args.use_split:
            print(
                f"----------------Number of tracklets before splitting: {len(tmp_trklets)}----------------"
            )
        ...
        print(
            f"----------------Number of tracklets before merging: {len(splitTracklets)}----------------"
        )
        ...
        print(
            f"----------------Number of tracklets after merging: {len(mergedTracklets)}----------------"
        )
        ...
        new_sct_output_path = os.path.join(
            data_path, sct_name, "{}.txt".format(seq_name)
        )
