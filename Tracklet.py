class Tracklet:
    def __init__(
        self, track_id=None, frames=None, scores=None, bboxes=None, feats=None
    ):
        """
        ...
        self.scores = (
            ...
            else [scores] if scores is not None else []
        )
        self.times = (
            frames
            if isinstance(frames, list)
            else [frames] if frames is not None else []
        )
        self.bboxes = (
            bboxes
            if isinstance(bboxes, list) and bboxes and isinstance(bboxes[0], list)
            else [bboxes] if bboxes is not None else []
        )
